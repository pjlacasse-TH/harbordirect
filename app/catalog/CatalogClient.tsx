...
function StockBadge({ status }: { status: 'in' | 'low' | 'out' }) {
  if (status === 'in') return (
    <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 px-2 py-0.5 rounded-full">
      In Stock
    </span>
  )
  if (status === 'low') return (
    <span className="text-xs font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 px-2 py-0.5 rounded-full">
      Low Stock
    </span>
  )
  return (
    <span className="text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-2 py-0.5 rounded-full">
      Out of Stock
    </span>
  )
}
...
function ProductCard({
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden hover:shadow-md transition-shadow">
      {/* Image */}
      <div className="relative bg-slate-100 dark:bg-slate-700 h-36 sm:h-44 flex items-center justify-center">
        {item.image_url ? (
          <Image
            src={item.image_url}
            alt={item.item_name}
        ...
        {item.sell_mode && (
          <span className="absolute top-2 left-2 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white/90 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-600 px-2 py-0.5 rounded-full capitalize">
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex flex-col gap-1.5">
          ...
          <h3 className="font-semibold text-[#0d2240] dark:text-blue-200 text-sm leading-snug line-clamp-2">
            {item.item_name}
          </h3>
        ...
        {sellBoth && (
          <div className="flex rounded-lg border border-slate-200 dark:border-slate-600 overflow-hidden text-xs font-medium self-start">
            <button
              ...
              className={`px-3 py-1.5 transition-colors capitalize ${mode === 'each' ? 'bg-[#0d2240] text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
              {item.sell_uom || 'Each'}
            </button>
            <button
              onClick={() => setMode('case')}
              className={`px-3 py-1.5 transition-colors ${mode === 'case' ? 'bg-[#0d2240] text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
            >
              Case
            </button>
          </div>
        ...
        <div className="mt-auto">
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-[#0d2240] dark:text-blue-200">{fmt(price)}</span>
        ...
        <div className="flex gap-2 items-center">
          <div className="flex items-center border border-slate-200 dark:border-slate-600 rounded-lg overflow-hidden text-sm select-none">
            <button
              ...
              className="px-2.5 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors disabled:opacity-30"
            >−</button>
            <span className="px-2.5 font-medium text-slate-800 dark:text-slate-200 min-w-[2rem] text-center">{qty}</span>
            <button
          ...
          <button
            ...
            className="flex-1 py-2.5 text-sm font-semibold rounded-lg transition-colors
              ...
              disabled:bg-slate-100 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:cursor-not-allowed"
          >
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-sm bg-white dark:bg-slate-800 z-50 shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            <ShoppingCartIcon className="w-5 h-5 text-[#0d2240] dark:text-blue-300" />
            <h2 className="font-semibold text-[#0d2240] dark:text-blue-200">Your Cart</h2>
          </div>
              ...
              {entries.map(([key, entry]) => (
                <li key={key} className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#0d2240] dark:text-blue-200 truncate">{entry.item.item_name}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 capitalize">{entry.mode} · {fmt(entry.unitPrice)} ea</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <button
                        onClick={() => onQty(key, -1)}
                        className="w-8 h-8 rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700"
                      >−</button>
                      <span className="text-sm font-medium w-4 text-center text-slate-800 dark:text-slate-200">{entry.qty}</span>
                      <button
                        onClick={() => onQty(key, 1)}
                        className="w-8 h-8 rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700"
                      >+</button>
                      <button
                        ...
                        className="ml-1 text-xs text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
                      >Remove</button>
                    </div>
                  </div>
                  <span className="text-sm font-semibold text-[#0d2240] dark:text-blue-200 whitespace-nowrap">
        ...
        {entries.length > 0 && (
          <div className="border-t border-slate-200 dark:border-slate-700 px-5 py-4 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-[#0d2240] dark:text-blue-200 text-lg">{fmt(total)}</span>
            </div>
            <button
              onClick={onCheckout}
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex flex-col gap-1.5">
          <StockBadge status={status} />
          <h3 className="font-semibold text-[#0d2240] dark:text-blue-200 text-sm leading-snug">{groupName}</h3>
          <p className="text-xs text-slate-400 dark:text-slate-500 font-mono">SKU: {item.sku}</p>
        </div>
                ...
                className={`relative px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${
                  isOut
                    ...
                    : isSelected
                      ...
                      : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-[#0d2240] hover:text-[#0d2240] dark:hover:text-blue-300 dark:hover:border-blue-400'
                }`}
              >
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            disabled={status === 'out' || price === null}
            onClick={() => { onAdd(item, mode, qty); setQty(1) }}
            className="flex-1 py-2.5 text-sm font-semibold rounded-lg transition-colors bg-[#0d2240] text-white hover:bg-[#1a3a6a] disabled:bg-slate-100 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:cursor-not-allowed"
          >
            {status === 'out' ? 'Out of Stock' : 'Add to Cart'}
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#1a4a8a] focus:border-transparent"
            />
          </div>
        </div>
      </div>
