...
// Order text is customer-entered and goes into document.write — escape it.
...
export default function PrintReceiptButton({ order, customerEmail }: Props) {
  ...
  const isTerms = brand.payment_mode === 'terms'
  const docTitle = isTerms ? 'Order Confirmation' : 'Payment Receipt'
  const company = brand.tagline || brand.brand_name
  ...
  function handlePrint() {
    ...
    const itemRows = items.map(li => `
      <tr>
        <td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;">
          <div style="font-weight:600;color:#1e293b;">${esc(li.item_name)}</div>
          <div style="font-size:11px;color:#94a3b8;margin-top:2px;">SKU: ${esc(li.sku)}</div>
        </td>
    ...
    const trackingLine = order.tracking_number
      ? `<p style="margin:4px 0 0;font-size:12px;color:#475569;">
           <strong>Tracking:</strong> ${esc(order.carrier ? order.carrier + ' ' : '')}${esc(order.tracking_number)}
         </p>`
      : ''

    const notesLine = order.notes
      ? `<p style="margin:8px 0 0;font-size:12px;color:#475569;"><strong>Notes:</strong> ${esc(order.notes)}</p>`
      : ''
...
<head>
  <meta charset="utf-8" />
  <title>${docTitle} ${esc(order.order_number)}</title>
  <style>
    :root { --brand: ${brand.primary_color}; --brand-hover: ${brand.primary_hover}; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px; padding-bottom: 24px; border-bottom: 2px solid var(--brand); }
    .brand { display: flex; align-items: center; gap: 10px; }
    .brand-text h1 { font-size: 18px; font-weight: 800; color: var(--brand); }
    .brand-text p { font-size: 12px; color: #64748b; margin-top: 2px; }
    .receipt-label { text-align: right; }
    .receipt-label h2 { font-size: 22px; font-weight: 700; color: var(--brand); }
    ...
    .total-value { text-align: right; font-weight: 800; font-size: 16px; color: var(--brand); width: 120px; }
    ...
    .print-btn { display: block; margin: 0 auto 28px; padding: 10px 28px; background: var(--brand); color: #fff; font-size: 14px; font-weight: 600; border: none; border-radius: 8px; cursor: pointer; }
    .print-btn:hover { background: var(--brand-hover); }
  </style>
</head>
<body>
  ...
  <div class="header">
    <div class="brand">
      ${brand.logo_url ? `<img src="${esc(brand.logo_url)}" alt="" style="height:32px" />` : brand.brand_name === 'HarborDirect' ? `<svg width="36" height="24" viewBox="0 0 60 36" fill="none">
        <path d="M4 18 Q15 8 30 18 Q45 28 56 18" stroke="var(--brand)" stroke-width="4" stroke-linecap="round" fill="none" opacity="0.4"/>
        <path d="M4 26 Q15 16 30 26 Q45 36 56 26" stroke="var(--brand)" stroke-width="4" stroke-linecap="round" fill="none"/>
      </svg>` : ''}
      <div class="brand-text">
        <h1>${esc(company)}</h1>
        <p>${esc(brand.brand_name)} Customer Portal</p>
      </div>
    </div>
  ...
  <div class="meta">
    <div class="meta-block">
      <p>Customer</p>
      <strong>${esc(customerEmail)}</strong>
    </div>
    <div class="meta-block">
      <p>Order Date</p>
  ...
  <p class="footer-note">
    ${esc(company)} &middot; Thank you for your order.<br/>
  ...
  return (
    <button
      onClick={handlePrint}
      className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-[var(--brand)] dark:hover:text-blue-300 transition-colors"
      title="Print or save receipt as PDF"
    >
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
      </svg>
      {isTerms ? 'Print' : 'Print Receipt'}
    </button>
  )
}
